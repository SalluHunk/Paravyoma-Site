...
interface ContactPageClientProps {
  /** Pre-selected inquiry type, passed from URL ?type= param by the server component */
  initialInquiry: InquiryType;
}

export function ContactPageClient({ initialInquiry }: ContactPageClientProps) {
  const [selectedInquiry, setSelectedInquiry] =
    React.useState<InquiryType>(initialInquiry);
  const formRef = React.useRef<HTMLDivElement>(null);

  // Auto-scroll to form when arriving with a pre-selected type (e.g. from CTA button)
  React.useEffect(() => {
    if (initialInquiry) {
      setTimeout(() => {
